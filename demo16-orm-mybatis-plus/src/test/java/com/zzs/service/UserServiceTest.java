...
/**
 * Desc:User Service 测试
 * @author zzs
 * @date 2022/3/26 19:26
 */
@Slf4j
public class UserServiceTest extends Demo16OrmMybatisPlusApplicationTests {
    ...
    public void testSaveList() {
        ...
        for (int i = 4; i < 14; i++) {
            String salt = IdUtil.fastSimpleUUID();
            User user = User.builder().name("testSave" + i).password(SecureUtil.md5("123456" + salt)).salt(salt).email("testSave" + i + "@zzs.com").phoneNumber("1730000000" + i).status(1).lastLoginTime(new DateTime()).build();
            userList.add(user);
        }
